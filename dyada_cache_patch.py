...
def _get_ancestry_by_index(self: RefinementDescriptor, hierarchical_index: int) -> list[int]:
...
def _cached_get_branch(self, index, is_box_index=True, hint_previous_branch=None):
    ...
    if not is_box_index:
        ...
        if keys:
            ...
            if pos >= 0:
                best = keys[pos]
                hint_idx = hint_previous_branch[0] if hint_previous_branch is not None else -1
                if best > hint_idx:
...
def _patched_is_old(discretization, markers, desired_dimensionwise_positions,
                    parent_of_next_refinement, parent_branch, old_index):
    descriptor = discretization.descriptor
    old_index_branch, _ = descriptor.get_branch(
        old_index, is_box_index=False,
...
def _patched_mapping_state_init(self, discretization, ancestry_branch_state,
                                starting_index=None):
